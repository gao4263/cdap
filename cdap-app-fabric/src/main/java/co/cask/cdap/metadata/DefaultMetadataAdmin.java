    ...
    return new MetadataSearchResponseV2(
      ...
      ImmutableSet.copyOf(
        AuthorizationUtil.isVisible(results.getResults(), authorizationEnforcer, authenticationContext.getPrincipal(),
                                    new Function<MetadataSearchResultRecordV2, EntityId>() {
                                      @Override
                                      public EntityId apply(MetadataSearchResultRecordV2 input) {
                                        return EntityId.getKnownParentEntityId(input.getMetadataEntity());
                                      }
                                    }, null)),
